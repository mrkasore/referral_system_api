...
async def create_referral_code(token: str = Depends(oauth2_scheme)):
...
async def get_referral_code_by_email(email: str, token: str = Depends(oauth2_scheme)):
    res = await db.get_referrer_by_email(email)
    return res

@router.get('/referrers/{referrer_id}/referrals')
async def get_referrals_by_id_referrer(referrer_id: int, token: str = Depends(oauth2_scheme)):
